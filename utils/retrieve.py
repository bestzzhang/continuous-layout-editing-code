...
def retrieve(target_name, outpath, num_class_images):
    ...
    client = ClipClient(url="https://knn.laion.ai/knn-service", indice_name="laion5B-H-14", num_images=num_images,  aesthetic_weight=0.1)
...
if __name__ == "__main__":
    args = parse_args()
    names = args.target_name.split("+")
    for name in names:
        retrieve(name, f"{args.outpath}/samples_{name}", args.num_class_images)
